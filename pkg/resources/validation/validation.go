...
func CheckForVerbs(rules []rbacv1.PolicyRule, fldPath *field.Path) *field.Error {
	for i := range rules {
		rule := rules[i]
		if len(rule.Verbs) == 0 {
			return field.Required(fldPath.Child("rules"), fmt.Sprintf("policyRules must have at least one verb: %s", rule.String()))
		}
	}
	return nil
}
