// Package admission holds definitions and function for admissionWebhook.
package admission
...
type ValidatingAdmissionHandler interface {
	...
	// A default configuration can be made using NewDefaultValidationWebhook(...)
...
type MutatingAdmissionHandler interface {
	...
	// A default configuration can be made using NewDefaultMutationWebhook(...)
...
// NewDefaultValidationWebhook creates a new ValidatingWebhook based on the WebhookHandler provided.
// The path set on the client config will be appended with the webhooks path.
// The return webhook will not be nil.
func NewDefaultValidationWebhook(handler WebhookHandler, clientConfig v1.WebhookClientConfig, scope v1.ScopeType) *v1.ValidatingWebhook {
...
// NewDefaultMutationWebhook creates a new MutatingWebhook based on the WebhookHandler provided.
// The path set on the client config will be appended with the webhooks path.
// The return webhook will not be nil.
func NewDefaultMutationWebhook(handler WebhookHandler, clientConfig v1.WebhookClientConfig, scope v1.ScopeType) *v1.MutatingWebhook {
	info := defaultWebhookInfo(handler, clientConfig, scope)
...
// canHandleOperation returns true if the give handler list the operation in the request as a supported operation.
